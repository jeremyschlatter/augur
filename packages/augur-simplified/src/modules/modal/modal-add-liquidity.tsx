...
import {
  ...
  formatPercent,
  onChainMarketSharesToDisplayShares,
...
const ModalAddLiquidity = ({
  ...
  const percentFormatted = useMemo(() => {
    ...
    const feePercent = amm?.feePercent ? amm?.feePercent : feeOption.value;
  ...
  const userPercentOfPool = useMemo(() => {
    ...
    if (rawSupply) {
      if (modalType === ADD) {
        const displaySupply = onChainMarketSharesToDisplayShares(
          rawSupply,
          cash?.decimals
        );
        userPercent = String(
  ...
  const LIQUIDITY_METHODS = {
    ...
    [ADD]: {
      ...
      confirmAction: async () => {
        ...
        const hasLiquidity =
          amm?.liquidity !== undefined && amm?.liquidity !== '0';
        doAmmLiquidity(
          account,
          amm,
          market.marketId,
